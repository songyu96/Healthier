...
应用还提供书本核心知识库：目前收录36个主题，覆盖七大营养素、维生素与矿物质、食物选择、一日搭配以及记录与安全，不再只围绕计算规则选题。内容按“书中明确、书中案例、应用推导、安全提示”标注，默认先展示结论、可执行判断和重要边界，原理、案例与引用位置可按需展开。由于源 EPUB 没有纸书页码映射，界面使用目录序号与全书百分比定位，不显示虚假页码。覆盖范围见 `docs/book-knowledge-coverage.md`。
