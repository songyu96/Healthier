...
当前内置库共 174 种食物，来源包括 USDA Standard Reference 28、FNDDS 2017–2018/2021–2023、中国食物成分表公开查询平台，以及少量透明通用配方。每个官方条目保存版本与可追溯编号或网页；配方估值展示原料重量并标记可靠性。来源缺项不会用 0 补齐，名称不足以判断配方时仍可记录食物组和重量，但营养值保持未知。用户覆盖值标记为 `USER` 并单独保存在 IndexedDB。
