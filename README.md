...
应用还提供“与当前应用相关的核心知识”：首批收录24个与现有规则直接相关的主题，不代表两本书的完整整理。内容按“书中明确、书中案例、应用推导、安全提示”标注，默认先展示结论、可执行判断和重要边界，原理、案例与引用位置可按需展开。由于源 EPUB 没有纸书页码映射，界面使用目录序号与全书百分比定位，不显示虚假页码。
