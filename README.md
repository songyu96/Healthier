...
个人使用的本地优先健康饮食 PWA。应用以仓库内两本《你是你吃出来的》为主要规则来源，支持快速记餐、HD1 餐食确认、每日目标计算、营养区间、下一餐建议、七日总结、加密备份和文件式跨设备接力。
...
- `src/sync.ts`：`HD-SYNC-1` 文件接力容器、稳定业务数据哈希和轻量版本冲突判断；不负责云上传或逐条实时同步。

当前内置注册表共 237 种食物，来源包括 USDA Standard Reference 28、FNDDS 2017–2018/2021–2023、中国食物成分表公开查询平台，以及少量透明通用配方。其中 181 项五项营养完整、34 项可计算部分营养，普通食物库和餐食选择只展示这 215 项可计算食物。新增的中国官方条目覆盖紫甘蓝、常见菌菇、生鲜与熟制鱼、腊肠、腊肉和火腿等。另有 22 项因名称或配方不明确而保持营养未知，仅为既有记录、旧备份和内部兼容保留，不再作为普通候选项。每个官方条目保存版本与可追溯编号或网页；来源缺项不会用 0 补齐，日评和周报按营养素分别标记覆盖。饮品统一记录主分类和糖状态，牛奶、豆浆及酒仍保留各自书本分类。用户覆盖值标记为 `USER` 并单独保存在 IndexedDB。
...
设置页可以导出和导入 `.hdsync` 加密同步包。手机与电脑各自仍使用本机 IndexedDB；切换设备前导出，另一台设备解密预览后整体恢复。应用会记录本机基线并提示版本冲突，但不会自动合并两台设备的数据。普通 `.hdbak` 加密备份继续保留，可用于网盘、U 盘或其他文件传输方式，因此不依赖 Supabase 等云服务。

同步状态、设备标识和恢复回滚点仅保留在本机，不进入业务备份或业务数据哈希。备份密码不会写入文件之外的任何位置；密码丢失后无法恢复密文。
