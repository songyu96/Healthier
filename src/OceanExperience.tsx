  ...
  return <div className="ocean-experience" ref={host}>
    {lightweight ? <OceanScene {...props} /> : <div className="ocean-viewport" aria-label="可交互海洋场景，使用下方按钮切换镜头，也可拖动或缩放">
      <SceneBoundary onFailure={onFailure}>
        <Suspense fallback={null}><OceanCanvas {...props} active={active} paused={paused} quality={quality} cameraRequest={cameraRequest} onReady={onReady} onFailure={onFailure} /></Suspense>
      </SceneBoundary>
      {!ready && <div className="ocean-loading" role="status"><span className="ocean-loading-ring" /><b>正在驶入海湾</b><span>首次进入需要加载场景与角色</span></div>}
      <div className="ocean-3d-heading"><span className="ocean-coordinate">OPEN WATER / 01</span><h2>晴空海湾</h2><span className="ocean-scene-tag">3D 体验样片 <i /> 微风 · 平缓浪面</span></div>
